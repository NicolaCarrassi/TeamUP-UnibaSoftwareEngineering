...
function IDinfo(id, type) {
    ...
    if (messenger != 0) {
        ...
        $.ajax({
            ...
            dataType: 'JSON',
            success: (data) => {
                if(data.user_avatar.substr(43,8) === 'https://'){
                    data.user_avatar = data.user_avatar.substr(43)
                }
...
$(document).ready(function () {
    ...
    $('body').on('change', ".upload-attachment", (e) => {
        ...
        reader.addEventListener('load', (ev) => {
            ...
            if (!file.type.match("image.*")) {
                ...
                sendCard.find('.attachment-preview').remove(); // older one
                sendCard.prepend(attachmentTemplate('image', file.name, ev.target.result));
            }
        });
    });
    ...
    $('.messenger-search').on('keyup', function (e) {
        ...
        let check = $.trim($(this).val()).length > 0 ? $('.messenger-search').trigger('focus') : $('.messenger-tab').hide() + $('.messenger-listView-tabs a[data-view="users"]').trigger('click');
    ...
    $('body').on('change', ".upload-avatar", (e) => {
        ...
        reader.addEventListener('load', (ev) => {
            ...
            if (!file.type.match("image.*")) {
                ...
                // if the file is an image
                $('.upload-avatar-preview').css('background-image', 'url("' + ev.target.result + '")');
            }
        });
    });
